        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="max-w-2xl reveal mb-16">
            ...
            <h2 className="font-display text-[#0a1a2f] text-3xl md:text-4xl lg:text-[2.6rem] leading-[1.05]" style={{ fontWeight: 800, letterSpacing: "-0.02em" }}>
              Ten disciplines, one dependable partner
            </h2>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6" data-testid="we-serve-grid">
            {WE_SERVE.map((c, i) => (
              <Link
                key={c.slug}
                to={`/we-serve/${c.slug}`}
                data-testid={`we-serve-card-${c.slug}`}
                className={`reveal reveal-delay-${(i % 3) + 1} group relative rounded-sm overflow-hidden bg-[#0a1a2f] min-h-[320px] flex`}
              >
                ...
                <div className="relative z-10 mt-auto p-7 w-full">
                  <span className="text-[11px] tracking-widest uppercase text-[#c8a25c] font-semibold">0{i + 1 > 9 ? "" : ""}{i + 1}</span>
                  ...
                  <p className="mt-4 text-[#c8a25c] text-xs font-semibold">{c.subs.length} sub-services</p>
                </div>
              </Link>
            ))}
          </div>
